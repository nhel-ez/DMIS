<center>
    <div class="table-box" id="cabinetTable">
        <table class="min-w-full divide-gray-200 scroll" id="CabinetTable">
            <thead>
                <tr>
                    <th class="px-6 show-cabinet bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">Cabinet Name</th>
                    <th class="px-6 show-cabinet bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">File Count</th>
                    <th class="px-6 show-cabinet bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">File Size</th>
                    <th class="px-6 show-cabinet bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">Date Created</th>
                    <th class="px-6 show-cabinet bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider" data-sortable = "false"></th>
                </tr>
            </thead>
            <tbody class="bg-transparent divide-gray-200">
                @csrf
                @if ($data->count())
                    @foreach ($data as $item)
                    <tr>
                        <td class="px-6 py-4 show-data text-sm whitespace-no-wrap">{{ $item->CabinetName }}</td>
                        <td class="px-6 py-4 show-data text-sm whitespace-no-wrap"></td>
                        <td class="px-6 py-4 show-data text-sm whitespace-no-wrap"></td>
                        <td class="px-6 py-4 show-data text-sm whitespace-no-wrap">{{ $item->created_at }}</td>
                        <td class="show-cabinet-option text-center text-sm">
                        <a class="view-button" href="{{ route('DrCabinet.show',$item['id']) }}">
                        <x-jet-button type="submit" style="border-radius:24px; background-color:white; width:112px; height:38px; font-size:13px; font-weight:500; text-align:center; opacity:75%;" class="btn2 btn--svg2 js-animated-button">
                            ...
                            @can('delete')
                                <form action ="{{ action('CabinetController@destroy', $item['id']) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
    ...
    <script>
        ...
        $(document).ready( function () {
            var cabinet = $('#CabinetTable').DataTable({
        ...
        $('#mySearchCabinet').keyup( function() {
            $('#cabinetTable').show();
            cabinet.search($(this).val()).draw();
        });

        console.log('#CabinetTable');
        });     
